    ...
    try {
      if (id == null) {
        ...
        if (response.statusCode == 200) {
          ...
          await NotificationService().scheduleTaskReminder(newTask['id'], content, fullDate);
        }
      } else {
        final url = Uri.parse('${ApiService.baseUrl}/tasks/edit/$id');
        await http.put(
          url,
          headers: {"Content-Type": "application/json"},
          body: jsonEncode({"content": content, "due_date": dateString}),
...
if (data['proactive_feedback'] != null && mounted) {
  ...
  Future<void> sendFeedback(int outcome) async {
    if (_jitaiEventId == null) {
      debugPrint("⚠️ No JITAI event_id stored — feedback skipped.");
      return;
    }
    ...
    try {
      final response = await http.post(
        ...
        headers: {"Content-Type": "application/json"},
        body: jsonEncode({
          "event_id": _jitaiEventId,
          "outcome": outcome,
        }),
      );
      if (response.statusCode == 200) {
        debugPrint("✅ Feedback sent: event=$_jitaiEventId, outcome=$outcome");
  ...
  Future<void> checkJitaiIntervention() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final String type = data['type'] ?? 'NONE';
        final String message = data['message'] ?? '';

        if (type == 'NONE' || !mounted) return;

        setState(() => _jitaiEventId = eventId);
      ...
      debugPrint("JITAI check failed: $e");
    }
  }
  ...
  Future<void> addToGoogleCalendar(String title, String dateString) async {
    try {
      ...
      if (!await launchUrl(calendarUrl,
  ...
  List<dynamic> get completedTasks =>
      tasks.where((t) => t['is_done']).toList();
  ...
  void _showTaskModal({Map<String, dynamic>? existingTask}) {
    final textController = TextEditingController(
        text: existingTask != null ? existingTask['content'] : "");
        ...
        return StatefulBuilder(builder: (context, setModalState) {
          ...
          return Container(
            ...
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(existingTask != null ? "Edit Plan" : "New Plan",
                    style: const TextStyle(
                        fontSize: 22,
                        fontWeight: FontWeight.bold,
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () {
                      if (textController.text.isNotEmpty) {
                        saveTask(textController.text, getFullDateTime(),
                            id: existingTask?['id']);
                        Navigator.pop(context);
                      }
                    },
                    child: Text(
                        existingTask != null ? "Update Plan" : "Save Plan",
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title:
            const Text("My Planner", style: TextStyle(color: Colors.white)),
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [
                          BoxShadow(
                              color:
                                  const Color(0xFF546E7A).withOpacity(0.3),
                              blurRadius: 15,
  ...
  Widget _buildTaskTile(Map<String, dynamic> task) {
    ...
    final bool isOverdue =
        due != null && due.isBefore(DateTime.now()) && !isDone;
      ...
      child: ListTile(
        ...
        trailing: PopupMenuButton<String>(
          ...
          onSelected: (value) {
            ...
            if (value == 'calendar') {
              addToGoogleCalendar(task['content'], task['due_date']);
            }
          },
